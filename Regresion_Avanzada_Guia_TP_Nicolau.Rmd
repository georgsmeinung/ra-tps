---
title: "Regresión Avanzada"
subtitle: "Guía de Trabajos Prácticos"
author: "Jorge Nicolau"
output:
   html_document:
     toc: yes
     code_folding: show
     toc_float: yes
     df_print: paged
     theme: united
     code_download: true
editor_options: 
  markdown: 
    wrap: none
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

<br><br>

```{r, echo=FALSE, include = FALSE}
# Configurar las opciones de numeración y origen de datos
# y cargar las librerías necesarias para el análisis.

options(scipen=999)
rm(list = ls())

script_path <- file.path(dirname(rstudioapi::getSourceEditorContext()$path), "")
setwd(script_path)

library(readxl)
library(readr)
library(ggplot2)
library(MVN)
library(gridExtra)
library(corrplot)
library(skimr)
library(aod)
library(lmtest)
library(car)
library(MASS)

```

# Capítulo 1 \| Regresión Lineal Simple

## [1.1 Correlación.]{style="color:darkred"}

### Ejercicio 1.1

**En el archivo grasacerdos.xlsx se encuentran los datos del peso vivo (PV, en Kg) y al espesor de grasa dorsal (EGD, en mm) de 30 lechones elegidos al azar de una población de porcinos Duroc Jersey del Oeste de la provincia de Buenos Aires.**

```{r, echo=TRUE}
# Cargar archivo de datos
grasacerdos <- read_excel("grasacerdos.xlsx")
# transformo comas a puntos
grasacerdos <- as.data.frame(lapply(grasacerdos, function(x) as.numeric(gsub(",", ".", x))))

head(grasacerdos)
```

**Se pide:**

-   **(a) Dibujar el diagrama de dispersión e interpretarlo.**

```{r, echo=TRUE}
# Calculo recta de regresión entre PV y EGD
lm_grasacerdos <- lm(EGD ~ PV, data = grasacerdos)
# Diagrama de dispersión
# con recta de regresión en color rojo
ggplot(grasacerdos, aes(x = PV, y = EGD)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión",
       x = "Peso Vivo (PV en Kg))",
       y = "Espesor de Grasa Dorsal (EGD en mm)") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
  
```

En el diagrama de dispersión, se observa la relación entre el peso vivo (PV) y el espesor de grasa dorsal (EGD) de los lechones. Cada punto representa un lechón, donde el eje x corresponde al peso vivo y el eje y al espesor de grasa dorsal. A simple vista, **no se aprecia una relación clara entre estas dos variables, ya que los puntos parecen dispersos sin seguir un patrón lineal evidente**.

-   **(b) Calcular el coeficiente de correlación muestral y explíquelo.**

```{r, echo=TRUE}
# Coeficiente de correlación muestral
coef_corr <- cor(grasacerdos$PV, grasacerdos$EGD)
print(paste("El coeficiente de correlación muestral es de", round(coef_corr, 2)))
```

El coeficiente de correlación muestral es de 0.25, lo que indica una correlación positiva débil entre el peso vivo y el espesor de grasa dorsal.

-   **(c) ¿Hay suficiente evidencia para admitir asociación entre el peso y el espesor de grasa? (α = 0,05). Verifique los supuestos para decidir el indicador que va a utilizar.**

Para determinar si existe suficiente evidencia para admitir una asociación entre PV y EGD en muestra, se puede realizar una prueba de hipótesis sobre el coeficiente de correlación de Pearson (𝑟) entre estas dos variables. Esta prueba permitirá evaluar si la correlación observada en la muestra es estadísticamente significativa, utilizando un nivel de significancia de α=0.05.

Para utlizar el test de correlación de Pearson, se deben verificar los siguientes supuestos:

-   **Normalidad de las variables**: El coeficiente de correlación de Pearson asume que ambas variables (PV y EGD) siguen una distribución normal. Esto se puede verificar mediante pruebas de normalidad como el test de Shapiro-Wilk o visualizando un histograma o un gráfico Q-Q de cada variable.

-   **Linealidad**: La relación entre PV y EGD debe ser aproximadamente lineal. Esto se puede verificar gráficamente con un diagrama de dispersión (scatter plot). Si la relación no es lineal, el coeficiente de correlación de Pearson podría no ser el mejor indicador. Si no se cumple el supuesto de linealidad, el coeficiente de correlación de Spearman es una alternativa adecuada. Este coeficiente es una medida de asociación no paramétrica, que evalúa la relación monótona entre dos variables sin asumir linealidad.

-   **Independencia de las observaciones**: Las observaciones deben ser independientes entre sí. Esto significa que los valores de PV y EGD de un individuo no deben depender de los valores de otro individuo.

-   **Homocedasticidad**: La varianza del espesor de grasa dorsal (EGD) debe ser aproximadamente constante a lo largo de los valores del peso vivo (PV). Esto también se puede verificar en el gráfico de dispersión.

```{r, echo=TRUE}
# Prueba de normalidad de las variables con el test de Shapiro-Wilk
# Prueba para PV
shapiro.test(grasacerdos$PV)
```

El test de Shapiro-Wilk para la variable PV arroja un p-valor de 0.6925, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de normalidad. Por lo tanto, **se puede asumir que la variable PV sigue una distribución normal**.

```{r, echo=TRUE}
# Prueba de normalidad de las variables con el test de Shapiro-Wilk
# Prueba para EGD
shapiro.test(grasacerdos$EGD)
```

El test de Shapiro-Wilk para la variable EGD arroja un p-valor de 0.9395, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de normalidad. Por lo tanto, **se puede asumir que la variable EGD sigue una distribución normal**.

Cumplido el supuesto de normalidad, se revisa el supuesto de linealidad mediante el gráfico de dispersión generado anteriormente. El grafico no muestra una relación lineal clara entre las variables PV y EGD, por lo que se podría considerar utilizar el coeficiente de correlación de Spearman en lugar del coeficiente de correlación de Pearson.

```{r, echo=TRUE}
# Coeficiente de correlación de Spearman
coef_corr_spearman <- cor(grasacerdos$PV, grasacerdos$EGD, method = "spearman")
print(paste("El coeficiente de correlación de Spearman es de", round(coef_corr_spearman, 2)))
```

El coeficiente de correlación de Spearman es de 0.17, lo que indica una correlación positiva débil entre el peso vivo y el espesor de grasa dorsal. Para determinar si esta correlación es estadísticamente significativa, es necesario realizar una prueba de hipótesis para el coeficiente de Spearman.

```{r, echo=TRUE}
# Prueba de hipótesis para el coeficiente de correlación de Spearman
cor.test(grasacerdos$PV, grasacerdos$EGD, method = "spearman")
```

El p-valor de la prueba de hipótesis para el coeficiente de correlación de Spearman es de 0.3785, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de que no hay correlación entre el peso vivo y el espesor de grasa dorsal. Por lo tanto, **no se puede afirmar que exista una asociación significativa entre estas dos variables**.

### Ejercicio 1.2

**Los datos del cuarteto de Anscombe se encuentran en el archivo anscombe.xlsx.**

```{r, echo=TRUE}
# Cargar archivo de datos
anscombe <- read_excel("anscombe.xlsx")
str(anscombe)

# Elimino la primera fila del dataset
anscombe <- anscombe[-1, ]

# transformo la columna I a x1, II a x2, III a x3, IV a x4,
# ...2 a y1, ...4 a y2, ...6 a y3 y ...8 a y4
names(anscombe) <- c("x1", "y1", "x2", "y2", "x3", "y3", "x4", "y4")

# convierto las columnas a numéricas
anscombe <- as.data.frame(lapply(anscombe, function(x) as.numeric(gsub(",", ".", x))))


```

**Se pide explorar los datos de la siguiente manera:**

-   **(a) Graficar los cuatro pares de datos en un diagrama de dispersión cada uno.**

```{r, echo=TRUE}
# Calculo recta de regresión para x1, y1
lm1 <- lm(y1 ~ x1, data = anscombe)

# Diagramas de dispersión para x1, y1
# con recta de regresión en color rojo
ggplot(anscombe, aes(x = x1, y = y1)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Conjunto 1: Diagrama de Dispersión para x1, y1",
       subtitle = "Distribución aproximadamente lineal",
       x = "x1",
       y = "y1") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```

```{r, echo=TRUE}
# Calculo recta de regresión para x2, y2
lm2 <- lm(y2 ~ x2, data = anscombe)

# Diagramas de dispersión para x2, y2
# con recta de regresión en color rojo
ggplot(anscombe, aes(x = x2, y = y2)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Conjunto 2: Diagrama de Dispersión para x2, y2",
       subtitle = "Distribución cuadrática",
       x = "x2",
       y = "y2") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )

```

```{r, echo=TRUE}
# Calculo recta de regresión para x3, y3
lm3 <- lm(y3 ~ x3, data = anscombe)

# Diagramas de dispersión para x3, y3
# con recta de regresión en color rojo
ggplot(anscombe, aes(x = x3, y = y3)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Conjunto 3: Diagrama de Dispersión para x3, y3",
       subtitle = "Distribución lineal con un punto atípico",
       x = "x3",
       y = "y3") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```

```{r, echo=TRUE}
# Calculo recta de regresión para x4, y4
lm4 <- lm(y4 ~ x4, data = anscombe)

# Diagramas de dispersión para x4, y4
# con recta de regresión en color rojo
ggplot(anscombe, aes(x = x4, y = y4)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Conjunto 4: Diagrama de Dispersión para x4, y4",
       subtitle = "Relación constante en x4 y valor atípico en y4",
       x = "x4",
       y = "y4") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```

-   **(b) Hallar los valores medios de las variables para cada par de datos.**
-   **(c) Hallar los valores de la dispersión para cada conjunto de datos.**
-   **(d) Hallar el coeficiente muestral de correlación lineal en cada caso.**

```{r, echo=TRUE}
# Eploración de los datos
# Recoletar data frame con las metricas
metricas <- data.frame(
  Conjunto = c("Conjunto 1", "Conjunto 2", "Conjunto 3", "Conjunto 4"),
  Correlacion = c(cor(anscombe$x1, anscombe$y1), cor(anscombe$x2, anscombe$y2), cor(anscombe$x3, anscombe$y3), cor(anscombe$x4, anscombe$y4)),
  Media_X = c(mean(anscombe$x1), mean(anscombe$x2), mean(anscombe$x3), mean(anscombe$x4)),
  Media_Y = c(mean(anscombe$y1), mean(anscombe$y2), mean(anscombe$y3), mean(anscombe$y4)),
  Desviacion_X = c(sd(anscombe$x1), sd(anscombe$x2), sd(anscombe$x3), sd(anscombe$x4)),
  Desviacion_Y = c(sd(anscombe$y1), sd(anscombe$y2), sd(anscombe$y3), sd(anscombe$y4)),
  Min_X = c(min(anscombe$x1), min(anscombe$x2), min(anscombe$x3), min(anscombe$x4)),
  Q1_X = c(quantile(anscombe$x1, 0.25), quantile(anscombe$x2, 0.25), quantile(anscombe$x3, 0.25), quantile(anscombe$x4, 0.25)),
  Mediana_X = c(median(anscombe$x1), median(anscombe$x2), median(anscombe$x3), median(anscombe$x4)),
  Q3_X = c(quantile(anscombe$x1, 0.75), quantile(anscombe$x2, 0.75), quantile(anscombe$x3, 0.75), quantile(anscombe$x4, 0.75)),
  Max_X = c(max(anscombe$x1), max(anscombe$x2), max(anscombe$x3), max(anscombe$x4)),
  IQR_X = c(IQR(anscombe$x1), IQR(anscombe$x2), IQR(anscombe$x3), IQR(anscombe$x4)),
  Min_Y = c(min(anscombe$y1), min(anscombe$y2), min(anscombe$y3), min(anscombe$y4)),
  Q1_Y = c(quantile(anscombe$y1, 0.25), quantile(anscombe$y2, 0.25), quantile(anscombe$y3, 0.25), quantile(anscombe$y4, 0.25)),
  Mediana_Y = c(median(anscombe$y1), median(anscombe$y2), median(anscombe$y3), median(anscombe$y4)),
  Q3_Y = c(quantile(anscombe$y1, 0.75), quantile(anscombe$y2, 0.75), quantile(anscombe$y3, 0.75), quantile(anscombe$y4, 0.75)),
  Max_Y = c(max(anscombe$y1), max(anscombe$y2), max(anscombe$y3), max(anscombe$y4)),
  IQR_Y = c(IQR(anscombe$y1), IQR(anscombe$y2), IQR(anscombe$y3), IQR(anscombe$y4))
)

metricas

```

-   **(e) Observar, comentar y concluir.**

Los cuatro conjuntos de datos del cuarteto de Anscombe presentan diferencias significativas en sus características, a pesar de tener estadísticas descriptivas similares. A continuación, se presentan algunas observaciones y conclusiones sobre cada conjunto de datos:

-  **Conjunto 1:** Este conjunto muestra una **relación lineal clara entre las variables x1 e y1**, con una correlación positiva (0.82). Los valores de x1 y y1 tienen una distribución aproximadamente normal, con una dispersión moderada en ambos ejes. La media de x1 es de 9.0 y la media de y1 es de 7.5. Los valores de x1 varían entre 4.0 y 14.0, mientras que los valores de y1 varían entre 3.10 y 10.84. La mediana de x1 es de 9.0 y la mediana de y1 es de 7.58. Los valores de x1 y y1 están distribuidos de manera uniforme en el rango intercuartílico. 

-  **Conjunto 2:** Este conjunto muestra una **relación cuadrática entre las variables x2 e y2**, con una correlación positiva (0.82). Los valores de x2 y y2 tienen una distribución no lineal, con una dispersión moderada en ambos ejes. La media de x2 es de 9.0 y la media de y2 es de 7.5. Los valores de x2 varían entre 4.0 y 14.0, mientras que los valores de y2 varían entre 3.10 y 10.84. La mediana de x2 es de 9.0 y la mediana de y2 es de 8.14. Los valores de x2 y y2 están distribuidos de manera uniforme en el rango intercuartílico. 

-  **Conjunto 3:** Este conjunto muestra una **relación lineal con un punto atípico entre las variables x3 e y3**, con una correlación positiva (0.82). Los valores de x3 y y3 tienen una distribución aproximadamente normal, con una dispersión moderada en ambos ejes. La media de x3 es de 9.0 y la media de y3 es de 7.5. Los valores de x3 varían entre 4.0 y 14.0, mientras que los valores de y3 varían entre 3.10 y 10.84. La mediana de x3 es de 9.0 y la mediana de y3 es de 7.11. Los valores de x3 y y3 están distribuidos de manera uniforme en el rango intercuartílico. 

-  **Conjunto 4:** Este conjunto muestra una **relación constante en x4 y un valor atípico en y4**, con una correlación positiva (0.82). Los valores de x4 y y4 tienen una distribución no lineal, con una dispersión moderada en ambos ejes. La media de x4 es de 9.0 y la media de y4 es de 7.5. Los valores de x4 varían entre 8.0 y 19.0, mientras que los valores de y4 varían entre 5.25 y 12.50. La mediana de x4 es de 8.0 y la mediana de y4 es de 7.04. Los valores de x4 y y4 están distribuidos de manera uniforme en el rango intercuartílico. 

Los conjuntos de datos del cuarteto de Anscombe demuestran cómo diferentes conjuntos de datos pueden tener estadísticas descriptivas similares pero comportamientos muy diferentes. **Es importante explorar y visualizar los datos para comprender mejor la relación entre las variables y evitar conclusiones erróneas basadas únicamente en estadísticas descriptivas.**

## [1.2 Modelo Lineal Simple.]{style="color:darkred"}

<br>

### Ejercicio 1.3

**El archivo peso_edad_colest.xlsx disponible en contiene registros correspondientes a 25 individuos respecto de su peso, su edad y el nivel de colesterol total en sangre.**

```{r, echo=TRUE}
# Cargar archivo de datos
peso_edad_colest <- read_excel("peso_edad_colest.xlsx")
str(peso_edad_colest)
```

**Se pide:**

-   **(a) Realizar el diagrama de dispersión de colesterol en función de la edad y de colesterol en función de peso. Le parece adecuado ajustar un modelo lineal para alguno de estos dos pares de variables?**

```{r, echo=TRUE}
# Calculo recta de regresión entre colesterol y edad
lm_colest_edad <- lm(colest ~ edad, data = peso_edad_colest)
# Diagrama de dispersión colesterol vs edad
# con recta de regresión en color rojo
ggplot(peso_edad_colest, aes(x = edad, y = colest)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Colesterol en función de la Edad",
       x = "Edad",
       y = "Colesterol Total") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```
```{r, echo=TRUE}
# Calculo recta de regresión entre colesterol y peso
lm_colest_peso <- lm(colest ~ peso, data = peso_edad_colest)
# Diagrama de dispersión colesterol vs peso
# con recta de regresión en color rojo
ggplot(peso_edad_colest, aes(x = peso, y = colest)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Colesterol en función del Peso",
       x = "Peso",
       y = "Colesterol Total") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```
A partir de lo observado en los diagramas de dispersión, se puede apreciar que **la relación entre el colesterol total y la edad parece ser más lineal que la relación entre el colesterol total y el peso**. Por lo tanto, **sería más adecuado ajustar un modelo lineal para el colesterol en función de la edad**.

-   **(b) Estime los coeficientes del modelo lineal para el colesterol en función de la edad.**

```{r, echo=TRUE}
# visualizar los coeficientes del modelo lineal entre colesterol y edad
lm_colest_edad 
```

-   **(c) Estime intervalos de confianza del 95% para los coeficientes del modelo y compare estos resultados con el test de Wald para los coeficientes. Le parece que hay asociación entre estos test y el test de la regresión?**

```{r, echo=TRUE}
# Intervalos de confianza para los coeficientes del modelo
confint(lm_colest_edad)
```
```{r, echo=TRUE}
# Test de Wald para los coeficientes del modelo
wald.test(b = coef(lm_colest_edad), Sigma = vcov(lm_colest_edad), Terms = 1:2)
```
El est de Wald para los coeficientes del modelo muestra que ambos coeficientes son significativamente diferentes de cero, lo que indica que hay una asociación significativa entre la edad y el colesterol total en sangre. Los intervalos de confianza del 95% para los coeficientes del modelo también confirman esta asociación, ya que no incluyen el valor cero.

El estadístico de Wald calcuado como:

$$ W = \frac{(\hat{\beta} - \beta_0)}{SE(\hat{\beta})} $$
Donde:
- W es el estadístico de Wald
- $\hat{\beta}$ es el coeficiente estimado
- $\beta_0$ es el valor nulo
- $SE(\hat{\beta})$ es el error estándar del coeficiente estimado

y los intervalos de confianza del 95% para los coeficientes del modelo son consistentes en indicar que hay una asociación significativa entre la edad y el colesterol total en sangre.

-   **(d) A partir de esta recta estime los valores de** $E(Y)\;\text{para}\;\ x=25\;\text{años y}\;x=48\;\text{años}\;$ **¿Podría estimarse el valor de** $E(Y)\;\text{para}\;\ x=80\;\text{años}$

``` {r, echo=TRUE}
# Estimar el valor de E(Y) para x = 25 años
x_25 <- 25
y_25 <- predict(lm_colest_edad, newdata = data.frame(edad = x_25))
print(paste("El valor estimado de E(Y) para x = 25 años es:", round(y_25, 2)))
```

``` {r, echo=TRUE}
# Estimar el valor de E(Y) para x = 48 años
x_48 <- 48
y_48 <- predict(lm_colest_edad, newdata = data.frame(edad = x_48))
print(paste("El valor estimado de E(Y) para x = 48 años es:", round(y_48, 2)))
```

``` {r, echo=TRUE}
# Estimar el valor de E(Y) para x = 80 años
x_80 <- 80
y_80 <- predict(lm_colest_edad, newdata = data.frame(edad = x_80))
print(paste("El valor estimado de E(Y) para x = 80 años es:", round(y_80, 2)))
```
Aunque el modelo lineal puede proporcionar una estimación de $E(Y)$ para edades de 25 y 48 años, **no sería apropiado estimar el valor de $E(Y)$ para una edad de 80 años**. Esto se debe a que el modelo lineal se basa en la relación observada entre la edad y el colesterol total en la muestra de datos disponible, y **extrapolar más allá de los valores observados (entre 20 y 60 años) podría llevar a predicciones inexactas y poco fiables**.

-   **(e) Testee la normalidad de los residuos y haga un gráfico para ver si son homocedásticos.**

``` {r, echo=TRUE}
# Test de normalidad de los residuos
shapiro.test(residuals(lm_colest_edad))
```
El test de Shapiro-Wilk para los residuos del modelo arroja un p-valor de 0.5175, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de normalidad. Por lo tanto, **se puede asumir que los residuos del modelo siguen una distribución normal**.
``` {r, echo=TRUE}
# Grafico de residuos vs valores ajustados
ggplot(data.frame(residuos = residuals(lm_colest_edad), valores_ajustados = fitted(lm_colest_edad)), aes(x = valores_ajustados, y = residuos)) +
  geom_point() +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Gráfico de Residuos vs Valores Ajustados",
       x = "Valores Ajustados",
       y = "Residuos") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )

``` 
El gráfico de residuos versus valores ajustados muestra que los residuos están distribuidos de manera aleatoria alrededor de la línea cero, lo que indica que los residuos son homocedásticos. Esto sugiere que el modelo lineal es adecuado para los datos y que no hay una relación sistemática entre los residuos y los valores ajustados.

### Ejercicio 1.4

**Una empresa desarrolló un sistema de energía solar para calentar el agua para una caldera que es parte del sistema de energía del proceso productivo. Existe el interés de controlar la estabilidad del sistema, para ello se monitorea el mismo y se registran los datos cada hora. Los datos se encuentran disponibles en el archivo energia.xlsx**

```{r, echo=TRUE}
# Cargar archivo de datos
energia <- read_excel("energia.xlsx")
str(energia)
# Elimino el acento del nombre de la columna Energía
names(energia)[2] <- "Energia"
```
-   **(a) Realizar el diagrama de dispersión y evaluar si un modelo de regresión lineal es adecuado.**

```{r, echo=TRUE}
# Calculo recta de regresión entre Hora y Energía
lm_energia <- lm(Energia ~ Hora, data = energia)
# Diagrama de dispersión para las variables Hora y Energía
# con recta de regresión en color rojo
ggplot(energia, aes(x = Hora, y = Energia)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Energía en función de la Hora",
       x = "Hora",
       y = "Energía") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```
En el gráfico de dispersión, se puede observar si hay una relación lineal clara entre las dos variables y si el modelo lineal se ajusta bien a los datos. En este caso, el diagrama de dispersión muestra una relación lineal entre la hora y la energía, lo que sugiere que un modelo de regresión lineal podría ser adecuado para estos datos.

-   **(b) Estimar un modelo lineal y verificar la normalidad de los residuos del mismo.**

```{r, echo=TRUE}
# Visualizar los coeficientes del modelo lineal entre Hora y Energía
lm_energia
```
```{r, echo=TRUE}
# Verificar la normalidad de los residuos
shapiro.test(residuals(lm_energia))
```
El test de Shapiro-Wilk para los residuos del modelo arroja un p-valor de 0.006752, lo que indica que hay suficiente evidencia para rechazar la hipótesis nula de normalidad. Por lo tanto, **los residuos del modelo no siguen una distribución normal**.

-   **(c) En caso de rechazar este supuesto buscar una transformación lineal para este modelo y aplicarla.**

```{r, echo=TRUE}
# Transformación logarítmica de la variable Energía
energia$log_Energia <- log(energia$Energia)
# Ajustar un nuevo modelo lineal con la variable transformada
lm_energia_log <- lm(log_Energia ~ Hora, data = energia)
# Verificar la normalidad de los residuos del nuevo modelo
shapiro.test(residuals(lm_energia_log))
```
El test de Shapiro-Wilk para los residuos del nuevo modelo arroja un p-valor de 0.1454, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de normalidad. Por lo tanto, **los residuos del modelo con la variable transformada logarítmicamente siguen una distribución normal**.

-   **(d) Realizar el análisis diagnóstico del nuevo modelo y estimar un intervalo de confianza y un intervalo de predicción para 27.5 hs con ambos modelos. Comparar los intervalos.**
```{r, echo=TRUE}
# Diagnosticar el nuevo modelo lineal con la variable transformada
plot(lm_energia_log)
```
Los gráficos de diagnóstico del nuevo modelo lineal con la variable transformada logarítmicamente muestran que los residuos están distribuidos de manera aleatoria alrededor de la línea cero, lo que indica que el modelo es adecuado para los datos y que no hay una relación sistemática entre los residuos y los valores ajustados.

```{r, echo=TRUE}
# Estimar el intervalo de confianza y el intervalo de predicción para 27.5 horas con el modelo original
x_27_5 <- 27.5
confint(lm_energia, newdata = data.frame(Hora = x_27_5))
```
El intervalo de confianza para la energía estimada en 27.5 horas con el modelo original es de 436.12578 a 546.852940. 

```{r, echo=TRUE}
# Estimar el intervalo de confianza y el intervalo de predicción para 27.5 horas con el modelo original
x_27_5 <- 27.5
predict(lm_energia, newdata = data.frame(Hora = x_27_5), interval = "prediction")
```
La predicción de la energía en 27.5 horas con el modelo original es de 505.8125, con un intervalo de predicción del 95% de 314.9688 a 696.6563.

```{r, echo=TRUE}
# Estimar el intervalo de confianza y el intervalo de predicción para 27.5 horas con el modelo transformado
# Dado que el modelo transformado regresiona el logaritmo de la variable Energía 
# es necesario aplicar la función exponencial a los límites de los intervalos.
x_27_5 <- 27.5
exp(confint(lm_energia_log, newdata = data.frame(Hora = x_27_5)))
```
El intervalo de confianza para la energía estimada en 27.5 horas con el modelo transformado es de 441.4543461 a 543.361800.
```{r, echo=TRUE}
# Estimar el intervalo de confianza y el intervalo de predicción para 27.5 horas con el modelo transformado
# Dado que el modelo transformado regresiona el logaritmo de la variable Energía 
# es necesario aplicar la función exponencial a los límites de los intervalos.
x_27_5 <- 27.5
exp(predict(lm_energia_log, newdata = data.frame(Hora = x_27_5), interval = "prediction"))
```
La predicción de la energía en 27.5 horas con el modelo transformado es de 497.3898, con un intervalo de predicción del 95% de 347.7179 a 711.4867. 

Los intervalos de confianza y de predicción para la energía estimada en 27.5 horas son similares entre el modelo original y el modelo transformado. Sin embargo, el modelo transformado puede ser preferible si los residuos del modelo original no siguen una distribución normal, ya que la transformación logarítmica puede ayudar a mejorar la normalidad de los residuos y la precisión de las predicciones.

## [1.4 Tratamiento de la heterocedasticidad.]{style="color:darkred"}

### Ejercicio 1.5

**Se obtuvieron datos históricos del mercado inmobiliario de una ciudad de Nueva Taipei, en Taiwan. La base es inmobiliaria.csv. Las características son:**

-   **edad:** Edad de la propiedad (en años).
-   **distancia:** La distancia a la estación de transporte más cercana (en metros).
-   **negocios:** Cantidad de negocios de conveniencia en las cercanías a una distancia realizable a pie.
-   **latitud:** Latitud de la ubicación de la propiedad (en grados).
-   **longitud:** Longitud de la ubicación de la propiedad (en grados).
-   **precio:** Precio por metro cuadrado (en miles de dólares)

**Se quiere investigar si el precio de las propiedades puede ser estimado en función de alguna de las variables disponibles.**

```{r, echo=TRUE}
# Cargar archivo de datos
inmobiliaria <- read_delim("inmobiliaria.csv", delim = ";",
                    col_types = cols(edad = col_double(),
                    distancia = col_double(),
                    negocios = col_double(),
                    latitud = col_double(),
                    longitud = col_double(),
                    precio = col_double()
                )
)
str(inmobiliaria)

```

-   **(a) Analizar si el precio depende de alguna de las variables.**

```{r, echo=TRUE}
# Calculo recta de regresión entre precio y distancia
lm_precio_distancia <- lm(precio ~ distancia, data = inmobiliaria)
# Diagrama de dispersión precio vs distancia
# con recta de regresión en color rojo
ggplot(inmobiliaria, aes(x = distancia, y = precio)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Precio en función de la Distancia",
       x = "Distancia",
       y = "Precio por Metro Cuadrado") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```
```{r, echo=TRUE}
# Calculo recta de regresión entre precio y edad
lm_precio_edad <- lm(precio ~ edad, data = inmobiliaria)
# Diagrama de dispersión precio vs edad
# con recta de regresión en color rojo
ggplot(inmobiliaria, aes(x = edad, y = precio)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Precio en función de la Edad",
       x = "Edad",
       y = "Precio por Metro Cuadrado") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```
```{r, echo=TRUE}
# Calculo recta de regresión entre precio y negocios
lm_precio_negocios <- lm(precio ~ negocios, data = inmobiliaria)
# Diagrama de dispersión precio vs negocios
# con recta de regresión en color rojo
ggplot(inmobiliaria, aes(x = negocios, y = precio)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Precio en función de la Cantidad de Negocios",
       x = "Cantidad de Negocios",
       y = "Precio por Metro Cuadrado") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```
```{r, echo=TRUE}
# Calculo recta de regresión entre precio y ubicación (latitud y longitud)
lm_precio_ubicacion <- lm(precio ~ latitud + longitud, data = inmobiliaria)
# Diagrama de dispersión precio vs ubicación
# diametro de los puntos proporcional al precio
# escalade color de los puntos proporcional al precio 
# de azul (bajo precio) a rojo (alto precio)
ggplot(inmobiliaria, aes(x = longitud, y = latitud, size = precio, color = precio)) +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Precio en función de la Ubicación",
       x = "Longitud",
       y = "Latitud") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )
```
La relación entre el precio de las propiedades y las variables disponibles analizadas mediante diagramas de dispersión y modelos de regresión lineal muestran que **la variable distancia parece tener una relación lineal con el precio de las propiedades**, mientras que las variables edad, cantidad de negocios y ubicación (latitud y longitud) no parecen tener una relación lineal clara con el precio.

-   **(b) Estudiar la linealidad de la relación precio-distancia.**

```{r, echo=TRUE}
# Test de Breusch-Pagan para la heterocedasticidad
bptest(lm_precio_distancia)
```
El test de Breusch-Pagan para la heterocedasticidad en el modelo de regresión entre el precio y la distancia arroja un p-valor de 0.2302, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de homocedasticidad. Por lo tanto, **se puede asumir que la relación entre el precio y la distancia es homocedástica**.

Dado que la relación entre el precio y la distancia es homocedástica, el modelo de regresión lineal es adecuado para estos datos y puede utilizarse para predecir el precio de las propiedades en función de la distancia a la estación de transporte más cercana.

-   **(c) Estimar los coeficientes del modelo y realizar el análisis diagnóstico de los residuos del mismo. Utilizar para este análisis los gráficos de residuos versus valores ajustados, el qq-plot de los residuos, la grafica de residuos versus leverage.**

```{r, echo=TRUE}
# Visualizar los coeficientes del modelo lineal entre precio y distancia
lm_precio_distancia
```
```{r, echo=TRUE}
# Diagnosticar el modelo lineal entre precio y distancia
plot(lm_precio_distancia)
```
Los gráficos de diagnóstico del modelo lineal entre el precio y la distancia muestran que los residuos están distribuidos de manera aleatoria alrededor de la línea cero, lo que indica que el modelo es adecuado para los datos y que no hay una relación sistemática entre los residuos y los valores ajustados.

-   **(d) Aplicar los test de Durbin-Watson Breush-Pagan.**

```{r, echo=TRUE}
# Test de Durbin-Watson para la autocorrelación de los residuos
durbinWatsonTest(lm_precio_distancia)
```
El test de Durbin-Watson para la autocorrelación de los residuos en el modelo de regresión entre el precio y la distancia arroja un valor de -0.08525486, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de no autocorrelación. Por lo tanto, **se puede asumir que los residuos del modelo no están autocorrelacionados**.

-   **(e) Analice la presencia de outlier y verifique si coinciden con los puntos influyentes.**

```{r, echo=TRUE}
# Identificar los puntos influyentes en el modelo
influential_points <- influence.measures(lm_precio_distancia)
summary(influential_points)
```

Para identificar los puntos influyentes en el modelo, se utilizó la función influence.measures() que proporciona información sobre los residuos estudentizados, los residuos de Cook y otros estadísticos de influencia. Los puntos influyentes se identifican por valores extremos en estos estadísticos, lo que indica que tienen un impacto significativo en el modelo de regresión.

Para identificar los outliers, se pueden considerar los puntos con residuos estudentizados o residuos de Cook extremadamente altos. Estos puntos pueden tener un impacto significativo en el modelo y pueden ser considerados outliers.

```{r, echo=TRUE}
plot(lm_precio_distancia, which = 4, main = "Cook's Distance")
abline(h = 4 / nrow(dataset), col = "red", lty = 2) # Línea de referencia
```
El gráfico de distancia de Cook muestra que hay varios puntos con valores extremadamente altos, lo que indica que estos puntos pueden ser considerados outliers y tener un impacto significativo en el modelo de regresión. Estos puntos pueden ser considerados como puntos influyentes y deben ser analizados en detalle para determinar si deben ser excluidos del modelo.

Se destacan directamente los puntos influyentes en los gráficos utilizando etiquetas:

```{r, echo=TRUE}
# Identificar los puntos influyentes en el modelo
library(ggplot2)

# Datos para el gráfico
influence_data <- data.frame(
  leverage = hatvalues(lm_precio_distancia),
  cooks_distance = cooks.distance(lm_precio_distancia),
  residuals = rstudent(lm_precio_distancia),
  id = rownames(inmobiliaria)
)

# Gráfico con etiquetas
ggplot(influence_data, aes(x = leverage, y = residuals)) +
  geom_point() +
  geom_text(aes(label = ifelse(cooks_distance > 4 / nrow(inmobiliaria), id, "")), 
            hjust = 1.2, color = "red") +
  theme_minimal() +
  labs(title = "Leverage vs. Residuals", x = "Leverage", y = "Standardized Residuals")
```


## [1.5 Cuadrados Mínimos Ponderados.]{style="color:darkred"}

Se visualizar los residuos estudentizados y los residuos de Cook para identificar los puntos influyentes y los outliers en el modelo de regresión entre el precio y la distancia.

-   **(f) Ajuste un modelo de regresión lineal ponderado por la distancia.**

```{r, echo=TRUE}
# Ajustar un modelo de regresión lineal ponderado por la distancia
lm_precio_distancia_ponderado <- lm(precio ~ distancia, data = inmobiliaria, weights = 1 / distancia)
# Visualizar los coeficientes del modelo ponderado
lm_precio_distancia_ponderado
```

El modelo de regresión lineal ponderado por la distancia ajustado es:

$$ Precio = 15.234572 -0.003441 \times \frac{1}{Distancia} $$
### Ejercicio 1.6

**En la base estudio.csv se encuentran registradas las horas de estudios referidas por un conjunto de estudiantes y su calificación en la evaluación final.**

```{r, echo=TRUE}
# Cargar archivo de datos
estudio <- read_delim("estudio.csv", delim = ";",
                    col_types = cols(horas_estudio = col_double(),
                    puntaje = col_double()
                )
)
str(estudio)
```

-   **(a) Ajuste un modelo de regresión simple para estimar la nota final en función de las horas dedicadas al estudio.**

```{r, echo=TRUE}
# Ajustar un modelo de regresión lineal simple entre puntaje y horas de estudio
lm_puntaje_estudio <- lm(puntaje ~ horas_estudio, data = estudio)
# Diagrama de dispersión puntaje vs horas de estudio
# con recta de regresión en color rojo
ggplot(estudio, aes(x = horas_estudio, y = puntaje)) +
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  labs(title = "Diagrama de Dispersión: Puntaje en función de las Horas de Estudio",
       x = "Horas de Estudio",
       y = "Puntaje") +
  theme_minimal() +
  theme(
    axis.line = element_line(size = 0.5)  # Keep axis lines
  )

```

-   **(b) Estudie el cumplimiento de los supuestos del modelo, gráfica y analíticamente.**

```{r, echo=TRUE}
# Visualizar los coeficientes del modelo lineal entre puntaje y horas de estudio
lm_puntaje_estudio
```
La relación entre el puntaje y las horas de estudio parece ser lineal, lo que sugiere que un modelo de regresión lineal simple podría ser adecuado para estos datos.

```{r, echo=TRUE}
# Test de Breusch-Pagan para la heterocedasticidad
bptest(lm_puntaje_estudio)
```

El test de Breusch-Pagan para la heterocedasticidad en el modelo de regresión entre el puntaje y las horas de estudio arroja un p-valor de 0.02877, lo que indica que hay suficiente evidencia para rechazar la hipótesis nula de homocedasticidad. Por lo tanto, **se puede asumir que la relación entre el puntaje y las horas de estudio es heterocedástica**.

```{r, echo=TRUE}
# Test de Durbin-Watson para la autocorrelación de los residuos
durbinWatsonTest(lm_puntaje_estudio)
```

El test de Durbin-Watson para la autocorrelación de los residuos en el modelo de regresión entre el puntaje y las horas de estudio arroja un valor de 1.826352 y un p-valor de 0.5, lo que indica que no hay suficiente evidencia para rechazar la hipótesis nula de no autocorrelación. Por lo tanto, **se puede asumir que los residuos del modelo no están autocorrelacionados**.

```{r, echo=TRUE}
# Identificar los puntos influyentes en el modelo
influential_points <- influence.measures(lm_puntaje_estudio)
summary(influential_points)
```

El único punto influyente en el modelo de regresión entre el puntaje y las horas de estudio es el punto 36, que tiene un alto valor de leverage y un alto valor de Cook's distance. Este punto puede tener un impacto significativo en el modelo y debe ser analizado en detalle para determinar si debe ser excluido del modelo.

```{r, echo=TRUE}
# Gráfico de residuos estudentizados vs valores ajustados
plot(lm_puntaje_estudio, which = 1, main = "Residuals vs Fitted")
```

El gráfico de residuos estudentizados versus valores ajustados muestra que los residuos están distribuidos de manera aleatoria alrededor de la línea cero, lo que indica que el modelo es adecuado para los datos y que no hay una relación sistemática entre los residuos y los valores ajustados.

```{r, echo=TRUE}
# Gráfico de qq-plot de los residuos
plot(lm_puntaje_estudio, which = 2, main = "Normal Q-Q")
```

El gráfico de qq-plot de los residuos muestra que los residuos siguen aproximadamente una distribución normal, lo que sugiere que el modelo es adecuado para los datos y que los residuos no se desvían significativamente de la normalidad.

```{r, echo=TRUE}
# Gráfico de residuos vs leverage
plot(lm_puntaje_estudio, which = 5, main = "Residuals vs Leverage")
```

El gráfico de residuos versus leverage muestra que los residuos están distribuidos de manera aleatoria alrededor de la línea cero, lo que indica que el modelo es adecuado para los datos y que no hay una relación sistemática entre los residuos y el leverage.

-   **(c) Ajuste un modelo de mínimos cuadrados ponderados definiendo los pesos de tal manera que las observaciones con menor varianza tengan más peso.**

```{r, echo=TRUE}
# Calculo de la varianza de los residuos
estudio$residuos <- residuals(lm_puntaje_estudio)

# Calcular la varianza de los residuos
estudio$var_col <- studres(lm_puntaje_estudio)^2

# Definir los pesos
estudio$weights <- 1 / estudio$var_col  # Inverso de la varianza

# Ajustar el modelo WLS
model_wls <- lm(y ~ x1 + x2, data = dataset, weights = weights)

# Visualizar los coeficientes del modelo WLS
summary(model_wls)
```


-   **(d) Realice el análisis diagnóstico del segundo modelo ajustado.**

<br>

-   **(e) Compare ambos ajustes realizados y concluya.**

<hr>

# Capítulo 2 \| Modelo Lineal Multivariado

<br>

## [2.1 Modelo Aditivo.]{style="color:darkred"}

<br>

### **Ejercicio 2.1:** Con el set de datos trees, disponible en la biblioteca dplyr de R, pretendemos ajustar un modelo que estimo el volumen (en pies cúbicos) de los árboles de cerezo en función de la longitud de su circunferencia (en pulgadas) y de su altura (en pies).

<br>

### (a) Visualizar la asociación entre las variables de a pares.

```{r, echo=TRUE}

```

<br>

### (b) Ajuste un modelo lineal simple para cada una de las dos predictoras disponibles.

```{r, echo=TRUE}

```

<br>

### (c) Realice un análisis diagnóstico en cada caso y señale en caso de haberlos puntos influyentes y outliers.

```{r, echo=TRUE}

```

<br>

### (d) Estime un intervalo de confianza para los coeficientes del modelo lineal estimado en cada caso.

```{r, echo=TRUE}

```

<br>

### (e) Ajuste un nuevo modelo sin la/s observaciones influyentes.

```{r, echo=TRUE}

```

<br>

### (f) Construya el intervalo de confianza y el de predicción del 95% para un árbol cuyo diámetro es 16.1 pulgadas.

```{r, echo=TRUE}

```

<br>

### (g) Ajuste un modelo utilizando conjuntamente las dos variables predictoras y compare este ajuste con el mejor de los modelos anteriores mediante un test de modelos anidados. Concluya.

```{r, echo=TRUE}

```

<br><br>

## [2.2 Modelo con Interacción.]{style="color:darkred"}

<br>

### **Ejercicio 2.2:** El departamento de ventas de una empresa quiere estudiar la influencia que tienen los distintos canales de publicidad sobre las ventas de un producto recién lanzado al mercado. Se dispone de un conjunto de datos que contiene los ingresos (en millones) conseguido por ventas en 200 regiones, así como la cantidad de presupuesto, también en millones, destinado a anuncios por radio, TV y periódicos en cada una de ellas. Los datos están disponibles en la base publicidad.xlsx.

<br>

### (a) Ajustar un modelo de regresión lineal simple para cada una de las variables predictoras por separado. Realizar a continuación el análisis diagnóstico de los modelos.

```{r, echo=TRUE}

```

<br>

### (b) Ajustar un modelo aditivo con las tres variables y decidir si alguna de ellas no es significativa (test de Wald).

```{r, echo=TRUE}

```

<br>

### (c) Ajustar los modelos de a pares y quedarse con el que mejor explique la variable respuesta utilizando el criterio de AIC, $$R^2$$ y Cp_Mallows.

```{r, echo=TRUE}

```

<br>

### (d) Grafique para el modelo seleccionado el plano de ajuste y evalue si le parece adecuado.

```{r, echo=TRUE}

```

<br>

### (e) Considere el mejor modelo pero ahora con interacción. Compare los modelos con y sin interacción.

```{r, echo=TRUE}

```

<br><br>

## [2.3 Regresoras Categóricas.]{style="color:darkred"}

<br>

### **Ejercicio 2.3:** Con la base de datos Salaries con 397 registros de 6 variables de la biblioteca carData de R.

-   **rank** factor con tres niveles : AssocProf, AsstProf y Prof.
-   **discipline** factor con dos niveles A (departamentos teóricos) o B (departamentos aplicados).
-   **yrs.since.phd** años transcurridos desde el doctorado.
-   **yrs.service** años de servicio.
-   **sex** factor con dos niveles Femenino y Masculino.
-   **salary** salario por nueve meses en dólares.

<br>

### (a) Ajustar un modelo lineal para estimar el salario en función del sexo.

```{r, echo=TRUE}

```

<br>

### (b) Ajustar un modelo lineal para estimar el salario en función de los años de servicio.

```{r, echo=TRUE}

```

<br>

### (c) Encontrar el modelo lineal que produzca el mejor ajuste con dos variables. Es necesario considerar interacción?

```{r, echo=TRUE}

```

<br>

### (d) Ajustar el modelo completo.

```{r, echo=TRUE}

```

<br>

### (e) Proponer un modelo y justificar que es mejor que el modelo completo. Realizar el análisis diagnóstico para este modelo.

```{r, echo=TRUE}

```

<br><br>

## [2.4 Regresión Polinómica.]{style="color:darkred"}

<br>

### **Ejercicio 2.4:** El conjunto de datos de Boston del paquete MASS recoge la mediana del valor de la vivienda en 506 áreas residenciales de Boston. Junto con el precio (medv), se han registrado 13 variables adicionales.

-   **crim:** ratio de criminalidad per cápita de cada ciudad.
-   **zn:** Proporción de zonas residenciales con edificaciones de más de 25.000 pies cuadrados.
-   **indus:** proporción de zona industrializada.
-   **chas:** Si hay río en la ciudad (= 1 si hay río; 0 no hay).
-   **nox:** Concentración de óxidos de nitrógeno (partes per 10 millón).
-   **rm:** promedio de habitaciones por vivienda.
-   **age:** Proporción de viviendas ocupadas por el propietario construidas antes de 1940.
-   **dis:** Media ponderada de la distancias a cinco centros de empleo de Boston.
-   **rad:** Índice de accesibilidad a las autopistas radiales.
-   **tax:** Tasa de impuesto a la propiedad en unidades de \$10,000.
-   **ptratio:** ratio de alumnos/profesor por ciudad.
-   **black:** 1000(Bk − 0,63)2 donde Bk es la proporción de gente de color por ciudad.
-   **lstat:** porcentaje de población en condición de pobreza.
-   **medv:** Valor mediano de las casas ocupadas por el dueño en unidades de \$1000s.

<br>

### (a) Utilizar una regresión polinómica de grado 2, otra de grado 5 y otra de grado 10 para estimar la variable medv en función de la variable lstat.

```{r, echo=TRUE}

```

<br>

### (b) Comparar estos dos modelos utilizando el criterio de R2, son mejores que un modelo lineal simple?

```{r, echo=TRUE}

```

<br>

### (c) Estudie la incorporación de otra de las variables al modelo seleccionado.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 2.5:** Con los datos_fifa que contienen 17907 registros correspondientes a 51 variables. Se identifican dos variables numéricas de interés:

-   **Overall:** Reputación y jerarquía internacional numérica del jugador.
-   **Valor:** Sería el valor económico internacional de los jugadores.

### Definiendo como la variable predictora Overall y como variable respuesta Valor, se pide:

<br>

### (a) Visualizar la relación entre ambas variables.

```{r, echo=TRUE}

```

<br>

### (b) Ajustar un modelo lineal simple.

```{r, echo=TRUE}

```

<br>

### (c) Ajustar un modelo lineal polinómico (seleccionar el grado adecuado).

```{r, echo=TRUE}

```

<br>

### (d) Definir la métrica RMSE y evalauar sobre un conjunto de validación los modelos ajustados.

```{r, echo=TRUE}

```

<br>

### (e) Realizar el análisis diagnóstico en cada caso.

```{r, echo=TRUE}

```

<br><br>

## [2.5 Modelo Robusto.]{style="color:darkred"}

<br>

### **Ejercicio 2.6:** La base de datos crime.xlsx, tiene 51 observaciones de las siguientes variables:

-   **state** vector de caracteres que representa al estado.
-   **violent** tasa de crímenes violentos por cada 100.000 habitantes.
-   **murder** variable numérica que indica la cantidad cada 100.000 habitantes de asesinatos.
-   **poverty** variable numérica que indica la proporción de habitantes que están por debajo del límite de pobreza.
-   **single** variable numérica que indica el porcentaje de familias que tiene un único padre a cargo de la misma.
-   **metro** variable numérica que indica la proporción de familias que habitan en áreas metropolitanas.
-   **white** porcentaje de población blanca.
-   **highschool** porcentaje de habitantes graduados de la escuela sencundaria.

<br>

### (a) Ajustar un modelos de regresión OLS y realizar analítica y gráficamente un análisis diagnóstico examinando leverage y distancias de Cook.

```{r, echo=TRUE}

```

<br>

### (b) Identificar las observaciones influyentes (recordar que 4/n es un valor de corte muy utilizado para las distancias de Cook). Ajustar un modelo OLS sin esas observaciones. Comparar los coeficientes estimados en ambos modelos.

```{r, echo=TRUE}

```

<br>

### (c) Generar una nueva variable con el valor absoluto de los residuos y señalar los diez residuos más altos. Coinciden con los valores influyentes?

```{r, echo=TRUE}

```

<br>

### (d) Ajustar un modelo de regresión robusta mediante mínimos cuadrados ponderados iterados (IRLS). El comando para ejecutar una regresión robusta está rlme n(library MASS)e. Se pueden utilizar varias funciones de ponderación en IRLS uar en primera instancia los pesos de Huber.

```{r, echo=TRUE}

```

<br>

### (e) Hacerlo ahora con los pesos de la función bicuadrada ( psi = psi.bisquare).

```{r, echo=TRUE}

```

<br>

### **Nota:** para aquellos que quieran profundizar en modelos robustos: La regresión robusta no aborda los problemas de heterogeneidad de la varianza para solucionar este problema se puede utilizar la librería sandwich. El ejemplo presentado utiliza M estimadores. Hay otras opciones de estimación disponibles en rlmy: mínimos cuadrados recortados usando ltsRegen de la biblioteca robustbasee y MM-estimadores usando rlm.

<br><br>

## [2.6 Regresión Cuantiles.]{style="color:darkred"}

<br>

### **Ejercicio 2.7:** En la base de datos USgirl de la biblioteca Brq de R, se encuentran 500 registros correspondientes a edad y peso de mujeres de Estados Unidos. Se pide:

<br>

### (a) Graficar los pesos versus las edades. Qué se puede apreciar en este diagrama de dispersión?

```{r, echo=TRUE}

```

<br>

### (b) Ajustar un modelo para la mediana y graficar.

```{r, echo=TRUE}

```

<br>

### (c) Ajustar un modelo para los cuartiles y graficar.

```{r, echo=TRUE}

```

<br>

### (d) Ajustar un modelo para los deciles y graficar.

```{r, echo=TRUE}

```

<br><br>

# Capítulo 3 \| Modelos Alternativos

<br>

## [3.1 Selección de Variables.]{style="color:darkred"}

<br>

### **Ejercicio 3.1:** Con los datos table.b3 de la librería MPV(Montgomery, D.C., Peck, E.A., and Vining, C.G. (2001)) de R donde se registran para 32 automóviles las siguientes variables:

-   **y** → Rendimiento en millas por galón
-   **x1** → Desplazamiento
-   **x2** → Potencia (pies-libras)
-   **x3** → Torque (pies-libras)
-   **x4** → Tasa de Compresión
-   **x5** → Relación del eje trasero
-   **x6** → Carburador (barriles)
-   **x7** → Número de velocidades de trasmisión
-   **x8** → Lontitud Total (pulgadas)
-   **x9** → Ancho (pulgadas)
-   **x10** → Peso (libras)
-   **x11** → Tipo de trasmisión (1=automática, 0=manual)

<br>

### (a) Ajustar el modelo saturado (que contiene a todas las varibles dependientes).

```{r, echo=TRUE}

```

<br>

### (b) Analizar a través del VIF la presencia de multicolinealidad.

```{r, echo=TRUE}

```

<br>

### (c) Realizar una selección de variables foward teniendo en cuenta el criterio de Akaike.

```{r, echo=TRUE}

```

<br>

### (d) Escribir la expresión del modelo seleccionado. Realizar un análisis diagnóstico del mismo.

```{r, echo=TRUE}

```

<br>

### (e) Realizar una selección backward teniendo en cuenta el criterio de $$R^2\;\text{ajustado}$$ Se selecciona el mismo modelo?

```{r, echo=TRUE}

```

<br>

### (f) Utilizando la función ols_step_all_possible de la biblioteca olsrr creada por Hebbali (2020) obtener todas las regresiones posibles. Elegir un único modelo visualizando gráficamente los resultados y considerando los criterios BIC, AIC, CP y $$R^2_{adj}$$

```{r, echo=TRUE}

```

<br>

### **Ejercicio 3.2:** Con el conjunto de datos fat de la biblioteca faraway de R, se registran la edad, el peso, la altura y 10 mediciones de la circunferencia corporal de 252 hombres. El porcentaje de grasa corporal de cada hombre se estimó con precisión mediante una técnica de pesaje bajo el agua. Las variables de la base son:

-   **brozek** porcentaje de masa grasa según Brozek
-   **siri** porcentaje de masa grasa según Siri
-   **density** densidad (gm/cm3)
-   **age** edad (años)
-   **weight** peso (lbs)
-   **height** estatura (inches)
-   **adipos** BMI = Peso/Estatura2 (kg/m2)
-   **free** peso libre de grasa (Brozek)
-   **neck** circunferencia del cuello (cm)
-   **chest** circunferencia del pecho (cm)
-   **abdom** circunferencia abdominal (cm
-   **hip** circunferencia de la cadera (cm)
-   **thigh** circunferencia del muslo (cm)
-   **knee** circunferencia de la rodilla (cm)
-   **ankle** circunferencia del tobillo (cm)
-   **biceps** circunferencia extendida del biceps (cm)
-   **forearm** circunferencia del antebrazo (cm)
-   **wrist** Circunferencia de la muñeca (cm)

<br>

### (a) Hallar el mejor modelo de regresión lineal con variable respuesta brozek utilizando entre 1 y 14 variables predictoras. Elegir el mejor considerando el criterio $$C_p\;\text{de Mallows y }R^2_{adj}$$

```{r, echo=TRUE}

```

<br>

### (b) Repetir considerando ahora la minimización del Error Cuadrático Medio del modelo usando validación cruzada leave one out.

```{r, echo=TRUE}

```

<br>

### (c) Inspeccionar gráficamente el MSE y decidir cuál es el mejor modelo. Interpretar los coeficientes del mismo.

```{r, echo=TRUE}

```

<br>

### (d) Coinciden las variables de los modelos elegidos con los diferentes criterios.

```{r, echo=TRUE}

```

<br><br>

## [3.2 Modelos de Regularización.]{style="color:darkred"}

<br>

### **Ejercicio 3.3:** Con los datos macroeconómicos longley de la biblioteca lars de R, que presentan alta colinealidad vamos a ajustar modelos de regularización. La base tiene 16 registros de 7 variables económicas observadas entre 1947 y 1962.

-   **GNP.deflator** deflactor implícito de precios
-   **GNP** PBI
-   **Unemployed**; número de desempleados
-   **Armed.Forces:** cantidad de personas en fuerzas armadas.
-   **Population:** población no institucionalizada con edad igual o superior a 14 años.
-   **Year:** año de registro.
-   **Employed:** número de personas en relación de dependencia.

<br>

### (1) Ajustar un modelo de Ridge para la variable respuesta Employed.

```{r, echo=TRUE}

```

<br>

### (2) Ajustar un modelo de Lasso para la variable respuesta Employed.

```{r, echo=TRUE}

```

<br>

### (3) Ajustar un modelo de Elastic Net para la variable respuesta Employed.

```{r, echo=TRUE}

```

<br>

### (4) Comparar los resultados obtenidos en los tres modelos.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 3.4:** Los datos prostata.xlsx disponibles en contiene 99 registros con una serie de medidas clínicas en hombres previas a una cirugía de próstata.

-   **volumen_pros:** volumen prostático.
-   **peso_pros:** peso de la próstata.
-   **edad** en años.
-   **log_hiperp_benig:** logaritmo de la hiperplasia benigna.
-   **invade_vesic_semin:** invasión de vesículas seminales.
-   **penetrac_capsular:** penetración capsular.
-   **gleason:** índice de Gleason.
-   **porc_punt_gleas_45:** proporción de puntuación 4 o 5.
-   **log_psa:** logaritmo del antígeno prostático.

<br>

### (a) Considerando la variable respuesta lpsa, ajustar un modelo lineal utilizando como predictoras a todas las demás. Qué inconveniente tiene este modelo?.

```{r, echo=TRUE}

```

<br>

### (b) Aplicar un método de selección de variables utilizando como criterio BIC. Qué variables quedaron? Coinciden con el OLS?.

```{r, echo=TRUE}

```

<br>

### (c) Ajustar ahora modelos regularizados y comparar los resultados y coeficientes utilizando CV.

```{r, echo=TRUE}

```

<br><br>

## [3.3 Modelos basados en PCA.]{style="color:darkred"}

<br>

### **Ejercicio 3.5:** Los dos conjuntos de datos están relacionados con variantes rojas y blancas del vino portugués "Vinho Verde"[Cortez et al., 2009]. Debido a cuestiones de privacidad y logística, solo están disponibles variables fisicoquímicas (entradas) y sensoriales (salida). Las clases están ordenadas y no equilibradas (por ejemplo, hay muchos más vinos normales que excelentes o malos). Los algoritmos de detección de valores atípicos podrían usarse para detectar los pocos vinos excelentes o malos. Además, no estamos seguros de si todas las variables de entrada son relevantes. Por lo que podría ser interesante probar métodos de selección de variables. Las bases de datos son winequality-white y winequality-red disponibles en shorturl.at/krty9 y shorturl.at/eqy39. Información de atributos:

-   acidez fija
-   acidez volátil
-   ácido cítrico
-   azúcar residual
-   cloruros
-   anhídrido sulfuroso libre
-   anhídrido sulfuroso total
-   densidad
-   pH
-   sulfatos
-   alcohol calidad (puntuación con rango 0-10)

Elija uno de los dos archivos y realice el siguiente análisis.

<br>

### (a) Realizar un correlograma para el conjunto de variables explicativas. Tiene sentido en este caso un PCA? En caso afirmativo explore las componentes principales.

```{r, echo=TRUE}

```

<br>

### (b) Partir la base en train-test. Considerando la calidad como variable respuestas, ajustar un modelo de PCR.

```{r, echo=TRUE}

```

<br>

### (c) Cuál es el número óptimo de componentes principales a considerar? Grafique las puntuaciones originales y las ajustadas por PCR.

```{r, echo=TRUE}

```

<br>

### (d) Calcular el MSE para este subconjunto de componentes.

```{r, echo=TRUE}

```

<br>

### (e) Realizar el ajuste en este caso con PLS. Comparar los resultados de ambos modelos.

```{r, echo=TRUE}

```

<br>

### (f) \* (para hacer en la unidad de regresión logística) Clasifique a los vinos como regulares (calidad\< 5) → 0, y buenos o muy buenos (calidad≥ 5) → 1. Ajuste un modelo de regresión logística para estimar la calidad del vino. Evalue la pertinencia del modelo.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 3.6:** Usaremos los datosChemicalManufacturingProcess de la biblioteca AppliedPredictiveModeling de R.

### Este conjunto de datos contiene información sobre un proceso de fabricación de productos químicos, en el que el objetivo es comprender la relación entre el proceso y el rendimiento del producto final resultante. La materia prima en este proceso se somete a una secuencia de 27 pasos para generar el producto farmacéutico final. El objetivo de este proyecto fue desarrollar un modelo para predecir el porcentaje de rendimiento del proceso de fabricación. El conjunto de datos consta de 177 muestras de material biológico para las que se midieron 57 características. Los predictores son continuos, de conteo, categóricos; algunos están correlacionados y otros contienen valores faltantes. Las muestras no son independientes porque los conjuntos de muestras provienen del mismo lote de material de partida biológico.

<br>

### (a) Realizar un análisis cuidadoso de las variables predictoras y una limpieza de la base.

```{r, echo=TRUE}

```

<br>

### (b) Aplicar PCR y PLS para predecir Yield (rendimiento) y comparar los resultados de ambos métodos.

```{r, echo=TRUE}

```

<br><br>

# Capítulo 4 \| Análisis de la Varianza

<br>

## [4.1 DCA.]{style="color:darkred"}

<br>

### **Ejercicio 4.1:** Un investigador estudio el contenido en sodio de las marcas de cerveza comercializadas en Capital Federal y Gran Buenos Aires. Para ello, seleccionaron las 6 marcas más prestigiosas del mercado y eligió botellas o latas de 500ml de cada marca seleccionada y midió el contenido en sodio (en miligramos) de cada una de ellas.

### Los resultados de este muestreo se presentan en la siguiente tabla:

| Marca 1 | Marca 2 | Marca 3 | Marca 4 | Marca 5 | Marca 6 |
|:-------:|:-------:|:-------:|:-------:|:-------:|:-------:|
|  24.4   |  10.2   |  19.2   |  17.4   |  13.4   |  21.3   |
|  22.6   |  12.1   |  19.4   |  18.1   |   15    |  20.2   |
|  23.8   |  10.3   |  19.8   |  16.7   |  14.1   |  20.7   |
|   22    |  10.2   |   19    |  18.3   |  13.1   |  20.8   |
|  24.5   |   9.9   |  19.6   |  17.6   |  14.9   |  20.1   |
|  22.3   |  11.2   |  18.3   |  17.5   |   15    |  18.8   |
|   25    |   12    |   20    |   18    |  13.4   |  21.1   |
|  24.5   |   9.5   |  19.4   |  16.4   |  14.8   |  20.3   |

<br>

### (a) Graficar la variable observada en los grupos y analizar la presencia de outliers y la igualdad grafica de las medias y las formas de las distribuciones.

```{r, echo=TRUE}

```

<br>

### (b) Calcular la media y el desvio de cada uno de los grupos. Le parece que se satisface el supuesto de homogeneidad?

```{r, echo=TRUE}

```

<br>

### (c) Establecer las hipótesis estadísticas de interés.

```{r, echo=TRUE}

```

<br>

### (d) Contrastar las hipótesis con un nivel $$α=0,05$$.

```{r, echo=TRUE}

```

<br>

### (e) Verificar el cumplimiento de los supuestos de normalidad y homocedasticidad.

```{r, echo=TRUE}

```

<br>

### (f) Si se verifican concluir en el contexto del problema.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 4.2:** Para comparar cuatro suplementos “de engorde” en bovinos para carne, se seleccionaron, al azar, cuarenta animales Hereford de iguales edad y sexo, y de pesos homogéneos para ser usados en un experimento.

-   **suplemento 1 (S1)** estuvo constituido por grano partido y fuente A
-   **suplemento 2 (S2)** por grano partido y fuente B
-   **suplemento 3 (S3)** por grano entero y fuente A
-   **suplemento 4 (S4)** por grano entero y fuente B.

### Se asignaron aleatoriamente 10 animales por suplemento, los que fueron alimentados individualmente con una dieta estándar más el correspondiente suplemento durante 80 días. La variable en estudio (o respuesta) fue la eficiencia de conversión (EfCon) individual (kg Materia Seca/ kg Ganancia de Peso) cuyos registros se presentan en la siguiente tabla:

| S1  | S2  | S3  | S4  |
|:---:|:---:|:---:|:---:|
| 3.3 | 4.6 | 6.7 | 6.3 |
| 4.4 | 4.5 | 5.8 |  6  |
| 4.9 |  5  |  5  | 6.7 |
| 4.9 |  4  | 4.8 | 5.5 |
| 3.9 | 4.5 | 5.3 | 6.6 |
| 4.2 | 5.2 | 6.2 | 6.1 |
| 4.7 | 4.9 |  5  | 5.3 |
| 5.1 | 5.5 | 6.4 | 6.5 |
| 4.6 | 4.8 | 5.9 | 6.3 |
| 4.5 | 5.3 | 5.4 | 6.8 |

<br>

### (a) Realice un análisis gráfico y descriptivo de la eficiencia de conversión lograda por los distintos suplementos.

```{r, echo=TRUE}

```

<br>

### (b) Establezca las hipótesis de interés del problema y explicite los supuestos necesarios.

```{r, echo=TRUE}

```

<br>

### (c) Testee las hipótesis con nivel de significación del 5%.

```{r, echo=TRUE}

```

<br>

### (d) Analice el cumplimiento de los supuestos del modelo.

```{r, echo=TRUE}

```

<br>

### (e) Concluya en términos del problema y si rechazó H0, indique cuales medias son diferentes. Utilice para ello las comparaciones a posteriori de Tuckey.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 4.3:** Se desea estudiar el efecto de una nueva droga analgésica para uso farmacéutico en pacientes con neuralgia crónica. Para ello se la compara con la aspirina y un placebo. En 30 pacientes elegidos al azar, se utiliza el método del doble ciego, asignando al azar 10 pacientes a cada tratamiento. La v.a. observada es el número de horas en que el paciente está libre de dolor después de haber sido tratado. Los resultados obtenidos fueron:

|          | Media |  DE  |
|:--------:|:-----:|:----:|
| Placebo  |  2.5  | 0.13 |
| Aspirina | 2.82  | 0.2  |
|  Droga   |  3.2  | 0.17 |

### Se tienen los p valores de la Prueba de Levene(p=0.18); Prueba de Shapiro - Wilks de los residuos del modelo ( p = 0,24). Se pide:

<br>

### (a) Identifique la variable dependiente y el factor de interes.

```{r, echo=TRUE}

```

<br>

### (b) Escriba el modelo, en general y en términos del problema.

```{r, echo=TRUE}

```

<br>

### (c) Analice los resultados de las pruebas de hipótesis para los supuestos del modelo.

```{r, echo=TRUE}

```

<br>

### (d) Plantee las hipótesis y construya la tabla de Anova sabiendo que $$SC_{error}=\sum_{i=1}^{k}(n_i-1)s_i^2$$

```{r, echo=TRUE}

```

<br>

### (e) Compare los tratamientos y utilizando un test t con nivel global 0.05 es decir que como son 3 comparaciones $$α=0,05/3$$ para cada una.

```{r, echo=TRUE}

```

<br>

### (f) Adicionalmente se indagó a los pacientes sobre efectos colaterales gástricos como respuesta al tratamiento. Los encuestados respondieron según una escala entre 0 y 5 (0 = nunca, 5= siempre). Los resultados obtenidos fueron:

|          |     |     |     |     |     |     |     |     |     |     |
|:--------:|:---:|:---:|:---:|-----|-----|-----|-----|-----|-----|-----|
| Placebo  |  0  |  3  |  2  | 3   | 4   | 2   | 2   | 3   | 1   | 1   |
| Aspirina |  1  |  4  |  3  | 0   | 2   | 3   | 4   | 5   | 2   | 3   |
|  Droga   |  4  |  5  |  4  | 2   | 3   | 4   | 1   | 5   | 3   | 0   |

### (I) ¿Cree que los investigadores deberían utilizar la misma prueba estadística que la empleada para comparar el tiempo libre de dolor? Justifique.

```{r, echo=TRUE}

```

### (II) ¿Cuáles son las conclusiones de este estudio?

```{r, echo=TRUE}

```

<br><br>

## [4.2 Alternativa no paramétrica.]{style="color:darkred"}

<br>

### **Ejercicio 4.4:** Se está estudiando el tiempo de cocción de un alimento antes de lanzarlo al mercado. Se han formado cuatro grupos y se les ha pedido que midan el tiempo transcurrido hasta que, según su juicio, el alimento quede a punto. Como esta sensación es subjetiva, se usa un ANOVA para estimar la varianza que presenta el experimento. Todos los grupos usan fuentes de calor y utensilios similares. Si la tabla siguiente recoge los resultados redondeados en minutos, ¿qué estimación podríamos hacer de la varianza de la población de estos alimentos? ¿Se observan diferencias entre los grupos?

| Grupo A | Grupo B | Grupo C | Grupo D |
|:-------:|:-------:|:-------:|:-------:|
|   25    |   121   |   81    |   25    |
|   36    |   36    |   81    |   25    |
|   36    |   36    |   36    |   36    |
|   25    |   64    |    9    |   25    |
|   36    |   36    |   25    |   36    |
|   16    |   81    |   36    |   25    |
|   25    |   49    |    9    |   25    |
|   36    |   25    |   49    |   25    |
|   49    |   64    |   169   |   25    |
|   36    |   49    |    1    |   25    |
|   25    |   121   |   81    |   25    |

<br>

### (a) Grafique los tiempos de cocción por tratamiento. Calcule las medidas resumen de los mismos.

```{r, echo=TRUE}

```

<br>

### (b) Establezca las hipótesis de interés, escriba el modelo detallando los supuestos.

```{r, echo=TRUE}

```

<br>

### (c) Realice la prueba y el diagnostico correspondiente. Son válidos los resultados de la prueba?

```{r, echo=TRUE}

```

<br>

### (d) Si respondió afirmativamente en c) concluya en el contexto del problema. Si concluyo negativamente intente una transformación de potencia conveniente para normalizar y/o homocedastizar la variable respuesta.

```{r, echo=TRUE}

```

<br>

### (e) Realice nuevamente la prueba si fuera necesario y el diagnóstico del modelo correspondiente. Concluya en términos del problema.

```{r, echo=TRUE}

```

<br>

### (f) Compare los resultados con los del test no paramétrico.

```{r, echo=TRUE}

```

<br><br>

## [4.3 ANOVA de dos vías con y sin interacción.]{style="color:darkred"}

<br>

### **Ejercicio 4.5:** Se pretende evaluar el efecto de la humedad en el suelo sobre la germinación de las semillas considerando el factor de cobertura. La base de datos semillas.xlsx contiene 48 registros de germinaciones con diferente porcentaje de humedad del suelo, con y sin cobertura del cultivo y proporción de germinación de las semillas.

<br>

### (a) Analice la proporción de germinación global.

```{r, echo=TRUE}

```

<br>

### (b) Estudie si hay asociación entre la humedad y la germinación.

```{r, echo=TRUE}

```

<br>

### (c) Analice si la germinación depende de la cobertura y si hay interacción entre los dos factores.

```{r, echo=TRUE}

```

<br>

### (d) Construya un modelo que permita explicar la relación de los dos factores con el porcentaje de germinación.

```{r, echo=TRUE}

```

<br>

### (e) Utilice los efectos y las comparaciones a posteriori para realizar una recomendación.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 4.6:** Se pretende estudiar la eficacia, medida por un score, de un medicamento considerando las combinaciones de dos factores: el género (masculino y femenino) y la edad categorizada (joven, adulto). Se quiere analizar si el efecto es diferente entre alguno de los niveles de cada variable por si sola o en combinación. Los datos están en el archivo eficacia.xlsx.

<br>

### (a) Explorar visualmente las medias por las distintas combinaciones de los factores considerados.

```{r, echo=TRUE}

```

<br>

### (b) Valorar visualmente la presencia de interacción.

```{r, echo=TRUE}

```

<br>

### (c) Construir un modelo y estimar los coeficientes del mismo. Interpretar los coeficientes y el efecto.

```{r, echo=TRUE}

```

<br><br>

# Capítulo 5 \| Regresión Logística

<br>

## [5.1 Modelo Univariado: interpretación.]{style="color:darkred"}

<br>

### **Ejercicio 5.1:** En 1986, el transbordador espacial Challenger tuvo un accidente catastrófico debido a un incendio en una de las piezas de sus propulsores. Era la vez 25 en que se lanzaba un transbordador espacial. En todas las ocasiones anteriores se habían inspeccionado los propulsores de las naves, y en algunas de ellas se habían encontrando defectos. El fichero challenger contiene 23 observaciones de las siguientes variables: defecto, que toma los valores 1 y 0 en función de si se encontraron defectos o no en los propulsores; y temp, la temperatura (en grados Fahrenheit) en el momento del lanzamiento. Los datos se encuentran en el fichero Challenger.xlsx.

<br>

### (a) ¿Se puede afirmarque la temperatura influye en la probabilidad de que los propulsores tengan defectos? Qué test utilizó para responder? Justifique.

```{r, echo=TRUE}

```

<br>

### (b) Interprete el valor del coeficiente estimado para temperatura en el contexto del problema.

```{r, echo=TRUE}

```

<br>

### (c) Para qué valores de temperatura la probabilidad estimada de que se produzcan defectos supera 0.1? y 0.5?

```{r, echo=TRUE}

```

<br>

### **Ejercicio 5.2:** Consideremos los datos del archivo diabetes.xlsx. Corresponden a 146 adultos que han participado de un ensayo sobre diabetes para investigar la relación entre la presencia de diabetes y varias medidas químicas. Es importante destacar que la obesidad se consideró un criterio de exclusión. El archivo contiene las siguientes variables explicativas:

-   **DIABET:** variable categórica 1 si es diabético y 0 si no.
-   **RELWT:** peso relativo.
-   **GLUFAST:** glucosa plasmática en ayunas.
-   **GLUTEST:** prueba de glucosa en plasma.
-   **INSTEST:** insulina plasmática durante la prueba.
-   **SSPG:** glucosa plasmática en estado estacionario.
-   **GROUP:** grupo clínico

<br>

### (a) Obtenga los box-plots para la variable SSPG para los grupos con y sin diabetes. Compare los valores medios de ambos grupos y comente.

```{r, echo=TRUE}

```

<br>

### (b) Obtenga el diagrama de dispersión de los valores observados de la variable respuesta (en el eje vertical) y la variable SSPG.

```{r, echo=TRUE}

```

<br>

### (c) Construya una tabla que contenga para cada grupo etáreo la media de la edad y la media de DIABET, que corresponde a la proporción de individuos que tienen DIABET en cada grupo. Analice.

```{r, echo=TRUE}

```

<br>

### (d) Ajuste un modelo para estimar diabet en función de SSPG.

```{r, echo=TRUE}

```

<br>

### (e) Interprete los coeficientes obtenidos en términos del problema planteado.

```{r, echo=TRUE}

```

<br>

### (f) Para una persona con SSPG igual a 100, ¿qué valores de logit, odds y la probabilidad de tener DIABET estima el modelo? Calcúlelos.

```{r, echo=TRUE}

```

<br>

### (g) Halle un intervalo de confianza del 95% del Odds Ratio para DIABET. Interprete.

```{r, echo=TRUE}

```

<br>

### (h) Evalúe la bondad del ajuste obtenido. Para ello construya una matriz de confusión con un conjunto de validación del 30% de los datos utilizando como punto de corte bp = 0,5. Hágalo luego con otros puntos de corte.

```{r, echo=TRUE}

```

<br>

### (i) Realizar el test de Hosmer –Lemeshow y comentar los resultados obtenidos.

```{r, echo=TRUE}

```

<br>

### (j) Estime el área bajo la curva ROC y explique el resultado.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 5.3:** Utilizaremos los datos que se encuentran en el archivo bajopeso. xlsx de un estudio, cuyo objetivo era identificar factores de riesgo asociados con el bajo peso al nacimiento (menor a 2500 gramos), se registraron las variables que se presentan en la siguiente tabla en 190 mujeres en el momento del parto. Los datos están disponibles en shorturl.at/dkpyZ.

-   **ID:** Código de Identificación
-   **LOW:** variable categórica que indica 1(bajo peso al nacer) 0(no).
-   **AGE:** edad de la madre en años.
-   **LWT:** peso de la madre en libras en el último periodo menstrual.
-   **RACE:** variable categórica con niveles: 1(blanca), 2(negra) y 3(otra.)
-   **SMOKE:** variable categórica con dos niveles 1(fumó durante el embarazo) y 0 (no).
-   **PTL:** cantidad de episodios de trabajo de parto prematuro.
-   **HT:** antecedentes de hipertensión 1 (si) y 0 (no).
-   **UI:** presencia de irritabilidad uterina 1(si) y 0(no).
-   **FTV:** cantidad de consultas durante el primer trimestre.

<br>

### (a) Estudie la relación entre bajo peso al nacer (LOW =1) y fumar durante el embarazo (SMOKE=1) mediante un modelo logístico.

```{r, echo=TRUE}

```

<br>

### (b) Escriba la expresión del modelo ajustado e interprete los coeficientes. Es significativa la variable smoke? Básese en el test deWald para responder a esta pregunta.

```{r, echo=TRUE}

```

<br>

### (c) Construya la matriz de confusión para un conjunto de validación de un tercio de la base.

```{r, echo=TRUE}

```

<br>

### (d) Testee basándose en la verosimilitud este modelo versus otro que considere también la edad de la madre. Interprete los resultados.

```{r, echo=TRUE}

```

<br>

<br><br>

## [5.2 Modelo Multivariado.]{style="color:darkred"}

<br>

### **Ejercicio 5.4:** El 4 de julio de 1999 una tormenta con vientos que excedían los 145 kilómetros por hora azotó el nordeste de Minnesota, en EEUU, causando graves daños en un parque natural de la zona. Los científicos analizaron los efectos de la tormenta determinando para más de 3600 árboles del parque, su diámetro en cm, una medida de la severidad local de la tormenta relacionada con el porcentaje inerte de área basal de cuatro especies, una variable que registraba si cada árbol había muerto (Y=1) o si había soportado la tormenta (Y=0) y finalmente la especie a la que pertenecía cada árbol. Estos datos se encuentran en el archivo tormenta.xlsx disponibles en shorturl.at/ACFHN. Fueron analizados por Weisberg et al. (2005).

<br>

### (a) Hay alguna especie que le parece que sobrevivió más que otra? Considera que la supervivencia se asocia con la severidad de la tormente? y con el diámetro del árbol?

```{r, echo=TRUE}

```

<br>

### (b) Proponga un modelo que sólo utilice como predictora la variable diámetro. Halle la bondad de ajuste y la precisión de la predicción lograda.

```{r, echo=TRUE}

```

<br>

### (c) Proponga un segundo modelo que considere como predicotras al diámetro y a la severidad de la tormenta. Halle la bondad de ajuste y la precisión de la predicción lograda.

```{r, echo=TRUE}

```

<br>

### (d) Compare ambos modelos considerando la verosimilitud, la bondad de ajuste y el área bajo la curva ROC de cada uno.

```{r, echo=TRUE}

```

<br>

### (e) Estimar la probabilidad de que no sobreviva un árbol cuyo diámetro es de 30 cm y esté situado en una zona en la que la fuerza de la tormenta viene dada por S=0.8.

```{r, echo=TRUE}

```

<br>

### (f) Compare la precisión del mejor de los modelos con un análisis discriminante lineal y con un análisis discriminante cuadrático.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 5.5:** Se ha conducido un estudio acerca de cáncer de próstata. Se analizó una base de una muestra aleatoria de 100 pacientes masculinos. Los datos corresponden al archivo prostata.xlsx. Sobre cada paciente se le han medido las siguientes variables o características:

-   **volumen_pros:** volumen de la próstata estimado.
-   **peso_pros:** peso de la próstata estimado.
-   **penetrac_capsular:** variable categórica que indica invasión de la cápsula.
-   **invade_vesic_semin:** variable categórica que indica invasión de la vesícula seminal.
-   **gleason:** Indicador de características de agresividad de las células.
-   **Edad:** edad del paciente.
-   **log_psa:** Logaritmo natural del indicador de antígeno prostático.

<br>

### (a) Construye un modelo de regresión logística a fin de establecer una relación que permita predecir la presencia de invasión de las vesículas seminales.

```{r, echo=TRUE}

```

<br>

### (b) Seleccione la/s variable/s adecuadas para informar cual/es de las variables incluidas en el modelo inicial son significativas.

```{r, echo=TRUE}

```

<br>

### (c) Escriba la expresión del modelo final si solo se incluyeran en el las variables que resultaron significativas.

```{r, echo=TRUE}

```

<br>

### (d) Pruebe interacciones dobles e incluya alguna en el modelo si resulta significativa.

```{r, echo=TRUE}

```

<br>

### (e) Considerando como valor de corte 0.5 cómo clasificaría un individuo que tuvo tiene Gleason 4 y penetración capsular.

```{r, echo=TRUE}

```

<br>

### **Ejercicio 5.6:** Una población de mujeres que tenían al menos 21 años de edad, descendientes de indígenas pima y que vivían cerca de Phoenix, Arizona, se sometieron a pruebas de diabetes de acuerdo con los criterios de la Organización Mundial de la Salud. Los datos fueron recopilados por el Instituto Nacional de Diabetes y Enfermedades Digestivas y Renales de EE. UU. Usamos los 532 registros completos después de descartar los datos (principalmente faltantes) sobre la insulina sérica. La base está disponible en la biblioteca Mass de R con el nombre Pima.tr. Las variables de la base son:

-   **npreg** cantidad de embarazos
-   **glu** concentración de glucosa plasmática en un test de tolerancia a la glucosa.
-   **bp** presión diastólica (mm Hg).
-   **skin** Grosor del pliegue cutáneo del tríceps (mm)
-   **bmi** índice de masa corporal (peso en kg/ estatura en m al cuadrado )
-   **ped** función de diabetes
-   **age** edad en años
-   **type** variable categórica de grupo de diabetes según el criterio de la OMS.

<br>

### (a) Ajustar un modelo logístico considerando como variable predictora el bmi y como respuesta type.Utilizar el test de razón de verosimilitudes para evaluar la significación del modelo, comparándolo con el modelo nulo.

```{r, echo=TRUE}

```

<br>

### (b) Defina una variable categórica que separe a las mujeres que no han tenido embarazos previos de las que sí. Ajuste un modelo para evaluar si esta variable es significativa para predecir type.

```{r, echo=TRUE}

```

<br>

### (c) Ajuste un modelo utilizando en este caso como predictoras la edad, la variable categórica definida en el item anterior y el bmi.

```{r, echo=TRUE}

```

<br>

### (d) Ajuste un modelo utilizando en este caso como predictoras la edad, el bmi y el número de embarazos previos.

```{r, echo=TRUE}

```

<br>

### (e) Seleccione el mejor modelo mediante el test de razón de verosimilitudes.

```{r, echo=TRUE}

```
